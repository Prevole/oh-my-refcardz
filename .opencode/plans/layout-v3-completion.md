...
- **8 skipped E2E tests in `e2e/layout-persistence.spec.ts`** : surfaced while running the full suite after the universals/help-settings fixes (`?`/`,` pre-pass + z-index + mutual exclusion). All 8 referenced the inert v2 keyboard hook. **Resolved post-H7** : 7 were deleted (obsolete with the V3 engine, fully covered by the new `keyboard-layout.spec.ts`, `layout-buffered-mode.spec.ts`, `layout-reset.spec.ts` and `layout-undo-redo.spec.ts` suites). The 8th ("remembers layout when navigating back from home") was rewritten as an SPA round-trip test (drag → Backspace to home → re-open sheet → assert positions match). Full suite now 131/131 with zero skip.
